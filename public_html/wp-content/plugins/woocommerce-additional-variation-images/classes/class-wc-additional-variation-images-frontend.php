	...
	public function load_images_ajax() {
		...
		if ( 0 < count( $image_ids ) ) {
			...
			foreach( $image_ids as $id ) {
				...
				$gallery_images .= apply_filters( 'woocommerce_single_product_image_thumbnail_html', sprintf( '<div class="col-md-3 col-sm-4 col-xs-6" style="margin-top:10px;"><a href="%s" class="%s" title="%s" %s %s>%s</a></div>', $image_link, $image_class, $image_title, $prettyPhoto, $cloudzoom, $gallery_image ), $id, $post_id, $image_class );
